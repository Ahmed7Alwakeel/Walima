...
const AltRecipesSection = ({ recipe, title, recipesSearchPage }) => {
...
useEffect(() => {
  ...
  return (
    <div className={`search-recipe-section ${recipesSearchPage ? 'recipesSearchPage' : ''} ${(recipe?.length==1 && isMobile) && "remove-padding"}`}>
      ...
      <h3 className='no-matching'>{t('search.no_matching_results')}</h3>
      <span className='no-matching-span'>{t('search.search_with_different_options')} {t('search.you_might_also_want_to_check')}</span>
      </div>
      {isMobile ?
...
export default AltRecipesSection
